...
def generate(query: str, context_docs: list, temperature: float = 0.1) -> str:
    """Build a RAG prompt from RRF-fused context documents and call the LLM.

    Args:
        query:        The user's natural-language question.
        context_docs: RRF-fused documents (each must have a ``"text"`` key).
    ...
    """
    log.debug(
        "Generating answer for query=%r using %d fused context documents",
        query[:80],
        len(context_docs),
    )
    context_block = "\n\n".join(
