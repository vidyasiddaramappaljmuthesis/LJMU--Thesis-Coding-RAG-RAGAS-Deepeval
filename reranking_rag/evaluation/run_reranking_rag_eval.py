"""
End-to-end Reranking RAG evaluation: RAGAS + DeepEval -> Excel
...
Each query makes exactly 1 Groq call (Step 3 answer generation).

Pipeline per query:
  Step 1a : ChromaDB initial retrieval  (bi-encoder, top-20 candidates, no LLM)
  Step 1b : Cross-encoder reranking     (rerank to top-5, no LLM)
...
Excel output (5 sheets):
  ...
  2. RAG Responses     -- retrieved contexts per query (reranked top-k + initial count)
...
Usage:
    conda run --no-capture-output -n rag_eval python reranking_rag/evaluation/run_reranking_rag_eval.py
    conda run --no-capture-output -n rag_eval python reranking_rag/evaluation/run_reranking_rag_eval.py --limit 5
    conda run --no-capture-output -n rag_eval python reranking_rag/evaluation/run_reranking_rag_eval.py --limit 10 --batch-size 2
"""
...
from reranking_rag.implementation.ingestion import build_vector_store, get_collection
...
from reranking_rag.implementation.config import (
    GROQ_API_KEYS, GROQ_MODEL, INITIAL_RETRIEVAL_K, TOP_K,
)

GOLDEN_PATH = PROJECT_ROOT / "dataset" / "golden" / "golden_dataset.csv"
RAG_NAME    = "Reranking-RAG"


def _default_output() -> Path:
    from datetime import datetime
    ts = datetime.now().strftime("%d-%m-%Y_%I-%M%p")   # e.g. 01-05-2026_12-30PM
    return PROJECT_ROOT / "reranking_rag" / "results" / f"{RAG_NAME}_{ts}.xlsx"
...
def _groq_chat(primary_idx: int, messages: list) -> str:
    """
...
def _evaluate_one(row, global_idx: int, total: int, primary_idx: int) -> tuple:
    ...
    Step 1a: ChromaDB initial retrieval  (top-20 candidates, no LLM)
    Step 1b: Cross-encoder reranking     (rerank to top-5, no LLM)
    Step 2 : Groq answer generation      (1 Groq call -- the only LLM call)
    Step 3 : RAGAS metrics               (reference-based, no LLM)
    Step 4 : DeepEval metrics            (reference-based, no LLM)
    ...
    # ── STEP 1a: ChromaDB Initial Retrieval ───────────────────────────────────
    buf.append(f"  +- STEP 1a . ChromaDB Initial Retrieval (top-{INITIAL_RETRIEVAL_K}) {'-' * max(0, _W - 48)}")
    initial_docs = []
    try:
        initial_docs = retrieve_initial(question, top_n=INITIAL_RETRIEVAL_K)
        buf.append(f"  |  Retrieved {len(initial_docs)} candidates  (cosine similarity, bi-encoder)")
        for i, d in enumerate(initial_docs[:5], 1):  # show first 5 for brevity
        ...
        if len(initial_docs) > 5:
            buf.append(f"  |    ... and {len(initial_docs) - 5} more candidates")
        buf.append(f"  +{'-' * (_W - 2)}")
    ...
    # ── STEP 1b: Cross-Encoder Reranking ──────────────────────────────────────
    buf.append(f"  +- STEP 1b . Cross-Encoder Reranking (top-{TOP_K}) {'-' * max(0, _W - 42)}")
    reranked_docs = []
    try:
        reranked_docs = rerank(question, initial_docs, top_k=TOP_K)
        buf.append(f"  |  Reranked {len(initial_docs)} -> {len(reranked_docs)} docs  "
                   f"(cross-encoder/ms-marco-MiniLM-L-6-v2)")
        for i, d in enumerate(reranked_docs, 1):
            rscore = d.get("rerank_score", 0)
            src    = d.get("metadata", {}).get("source", d.get("id", ""))
            snip   = d["text"].replace("\n", " ")[:100]
            buf.append(f"  |    [{i}] rerank_score={rscore:.4f}  src={src}")
            buf.append(f"  |         +- {snip}...")
        buf.append(f"  +{'-' * (_W - 2)}")
    except Exception as exc:
        buf += [f"  |  ERROR: {exc}", f"  +{'-' * (_W - 2)}"]
        reranked_docs = initial_docs[:TOP_K]   # graceful fallback
        buf.append(f"  |  [WARN]  Reranking failed — falling back to cosine top-{TOP_K}")
    ...
    try:
        answer = _generate_answer(question, reranked_docs, primary_idx)
    ...
    rag_r = {
        ...
        "retrieved_contexts":    [d["text"] for d in reranked_docs],
        "retrieved_context_ids": [d.get("id", "") for d in reranked_docs],
        "expected_context":      exp_ctx,
        "expected_source_ids":   exp_src_ids,
        "initial_docs":          initial_docs,
        "initial_context_ids":   [d.get("id", "") for d in initial_docs],
    }
    ...
    buf.append(f"  |  Reranked doc IDs    : {rag_r['retrieved_context_ids']}")
    ...
    buf += [
        ...
        f"  |  key #{primary_idx + 1}  |  Initial({INITIAL_RETRIEVAL_K})"
        f" -> Rerank({TOP_K}) -> Generate -> RAGAS -> DeepEval",
        "",
    ]
...
def evaluate_all(golden_df: pd.DataFrame, batch_size: int = 20,
    ...
    print(f"\n{'#' * _W}")
    print(f"#  END-TO-END RERANKING RAG EVALUATION  --  {n_selected} queries selected"
          f"  (dataset size: {total_dataset})")
    print(f"#  Pipeline         : ChromaDB(top-{INITIAL_RETRIEVAL_K}) -> CrossEncoder -> top-{TOP_K} -> Groq")
    print(f"#  LLM (generation) : {GROQ_MODEL}")
...
def export_excel(rag_results, ragas_df, deepeval_df, eval_mode, output_path):
    ...
    summary_rows = [{
        ...
        "Framework": "META", "Metric": "Reranker model",
        "Mean": "cross-encoder/ms-marco-MiniLM-L-6-v2", "Min": "", "Max": "", "Std": "", "N": "",
    }, {
        "Framework": "META", "Metric": "Initial retrieval k",
        "Mean": INITIAL_RETRIEVAL_K, "Min": "", "Max": "", "Std": "", "N": "",
    }, {
        "Framework": "META", "Metric": "Final top-k",
        "Mean": TOP_K, "Min": "", "Max": "", "Std": "", "N": "",
    }]
    for col in ragas_metric_cols:
    ...
    for r in rag_results:
        ...
        row = {
            ...
            "expected_answer":    r["expected_answer"],
            "generated_answer":   r["generated_answer"],
            "initial_pool_size":  len(r.get("initial_docs", [])),
            "reranked_pool_size": len(ctx),
        }
        for i in range(5):
            row[f"reranked_context_{i+1}"] = ctx[i] if i < len(ctx) else ""
        rag_rows.append(row)
